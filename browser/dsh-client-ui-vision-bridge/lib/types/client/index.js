 ...
 */
export function recognitionRequestTimeoutMs(config, imageCount) {
 ...
 */
export function canUploadUncompressedImage(data) {
...
async function recognizeViaServer(originalFetch, config, images, question) {
    ...
    try {
        ...
        if (!Array.isArray(payload.results) || payload.results.length !== prepared.length)
            return undefined;
        return {
...
export function apply(ctx) {
    ...
    const patchedFetch = async (input, init) => {
        ...
        if (!pathname.endsWith('/api/session.prompt'))
            return originalFetch(input, init);
        ...
        if (texts.trim().length > 0)
            rewritten.push({ type: 'text', text: texts.trim() });
    ...
    ctx.effect(() => () => {
        ...
        if (pill !== null) {
            ...
            if (Number.isFinite(ticker) && ticker > 0)
